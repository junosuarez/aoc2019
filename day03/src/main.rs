...
// return length from origin when path intersects given point
fn get_length_at(path: &String, point: &Point) -> i32 {
  let mut pen = Point { x: 0, y: 0 };
  let mut length = 0;
...
mod tests {
  ...
  #[test]
  fn ex_1() {
    let expected: HashSet<Point> = [Point { x: 1, y: 0 }, Point { x: 2, y: 0 }]
      .iter()
      .cloned()
      .collect();
    assert_eq!(expected, parse("R2".to_string()));
  }
  #[test]
  fn ex_2() {
    let p1 = "R75,D30,R83,U83,L12,D49,R71,U7,L72";
    let p2 = "U62,R66,U55,R34,D71,R55,D58,R83";
    let s1 = parse("R75,D30,R83,U83,L12,D49,R71,U7,L72".to_string());
    let s2 = parse("U62,R66,U55,R34,D71,R55,D58,R83".to_string());

    let intersections = s1.intersection(&s2);
    println!("intersections: {:?}", intersections);
    for i in intersections.clone() {
      println!("{:?} len {}", i, total_distance(p1, p2, i));
    }
    // assert_eq!(610, total_distance(path1: &str, path2: &str, point: &Point))
  }
  #[test]
  fn ex_3() {
    unimplemented!();
